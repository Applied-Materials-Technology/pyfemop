...
class MooseOptimisationRun():
    ...
    def __init__(self,name : str,optimisation_inputs : OptimisationInputs,herd : MooseHerd,cost_function : CostFunction,data_filter = None):
        ...
        # Setup algorithm
        self._optimisation_inputs._algorithm.setup(self._problem,termination=optimisation_inputs._termination)
    ...
    def run(self,num_its):
        ...
        for n_gen in range(num_its):
            ...
            if not self._optimisation_inputs._algorithm.has_next():
                # Kill the loop if the algorithm has terminated.
                break
            print('************************************************')
            cur_gen = self._optimisation_inputs._algorithm.n_gen
            if cur_gen is None:
                cur_gen = 1
            ...
            #Get parameters
            pop = self._optimisation_inputs._algorithm.ask()
            x = pop.get("X")
            
            if self._optimisation_inputs._run_type == 'standard':
